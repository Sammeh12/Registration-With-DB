...
namespace RegistrationWithDB
{
    class ClubRegistrationQuery
    {
        ...
        {
            connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\GAMAY\source\repos\RegistrationWithDB\RegistrationWithDB\ClubDB.mdf;Integrated Security=True";
        ...
        public bool DisplayList()
        {
            ...
            bindingSource.DataSource = dataTable; return true;
        }
        public bool RegisterStudent(int ID, long StudentID, string FirstName, string MiddleName, string LastName, int Age, string Gender, 
            string Program)
        {
            sqlCommand = new SqlCommand("Insert Into ClubMembers VALUES(@ID, @StudentID, @FirstName, " +

            "@MiddleName, @LastName, @Age, @Gender, @Program)", sqlConnect); sqlCommand.Parameters.Add("@ID", SqlDbType.Int).Value = ID; 
        ...
        {
            string getId = "SELECT * FROM ClubMembers"; sqlCommand = new SqlCommand(getId, sqlConnect); sqlConnect.Open();
            sqlReader = sqlCommand.ExecuteReader(); while (sqlReader.Read())
            {
        ...
        {
            string getId = "SELECT * FROM ClubMembers WHERE StudentId = @Id"; sqlCommand = new SqlCommand(getId, sqlConnect); 
